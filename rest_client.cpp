...
int RestClient::request(const char* method, const char* path,
  ...
  if(client.connect(hostAddress, port)){
    ...
    // if the host address is different now, try it again
    if (getHostAddress())
    {
        HTTP_DEBUG_PRINT("DNS address changed, retrying...\n");
        return request(method, path, body, response);
    }
...
bool RestClient::getHostAddress()
{
    IPAddress newAddress;
    int ret = dnsClient.getHostByName(host, newAddress);
    if (ret != 1)
    {
        HTTP_DEBUG_PRINT("Get host by name failed with status: ");
        HTTP_DEBUG_PRINT(ret);
        HTTP_DEBUG_PRINT("\n");
    }
    else
    {
        if (!(newAddress == hostAddress))
        {
            HTTP_DEBUG_PRINT("DNS address changed to ");
            HTTP_DEBUG_PRINT(newAddress);
            HTTP_DEBUG_PRINT("\n");
